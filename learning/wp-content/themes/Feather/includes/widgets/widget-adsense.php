<?php class AdsenseWidget extends WP_Widget
{
	function AdsenseWidget(){
		$widget_ops = array( 'description' => esc_html__( 'Displays Adsense Ads', 'Feather' ) );
		$control_ops = array( 'width' => 400, 'height' => 500 );
		parent::WP_Widget( false, $name = esc_html__( 'ET Adsense Widget', 'Feather' ), $widget_ops,$control_ops );
	}

	/* Displays the Widget in the front-end */
	function widget( $args, $instance ){
		extract($args);
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? esc_html__( 'Adsense', 'Feather' ) : esc_html( $instance['title'] ) );
