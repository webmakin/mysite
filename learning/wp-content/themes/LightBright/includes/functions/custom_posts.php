...
add_action('init', 'et_note_register');
...
function et_lightrbright_custom_metaboxes(){
...
add_action("admin_init", "et_lightrbright_custom_metaboxes");
...
function et_customlink_meta() {
	...
	<input name="customlink" id="customlink" type="text" style="width: 90%;" value="<?php echo esc_attr($customlink); ?>" /></p>
	<?php
}
...
function et_audio_meta() {
	...
	<input name="audio-url" id="audio-url" type="text" style="width: 90%;" value="<?php echo esc_attr($audio); ?>" /></p>
	<?php
}
...
function et_custom_post_save_details($post_id){
	...
	if (isset($_POST["thumb"])) update_post_meta($post_id, "thumb", esc_attr($_POST["thumb"]));
	...
	if (isset($_POST["video_width"])) update_post_meta($post_id, "video_width", esc_attr($_POST["video_width"]));
	if (isset($_POST["video_height"])) update_post_meta($post_id, "video_height", esc_attr($_POST["video_height"]));
	if (isset($_POST["customlink"])) update_post_meta($post_id, "customlink", esc_url($_POST["customlink"]));
	if (isset($_POST["audio-url"])) update_post_meta($post_id, "audio", esc_attr($_POST["audio-url"]));
}
